...
Two subscription lanes:
  ...
  - Claude (personal) -> `claude` CLI in headless mode (`-p`). The subprocess
    uses the machine-default login (the former ELVTR team subscription was
    cancelled and its CLAUDE_CONFIG_DIR pin removed). Serves
...
# The ELVTR team subscription (config in ~/.claude-elvtr) was cancelled; calls
# now run on the personal account, the machine-default `claude` login (_run
# scrubs CLAUDE_CONFIG_DIR so nothing can re-route to the dead team login).

DEFAULT_TIMEOUT = 420  # seconds; Opus and Gemini Pro on large inputs can be slow

# Agent -> assigned model and subscription lane.
#   provider "claude" : served by the `claude` CLI (Claude (personal))
...
AGENT_REGISTRY = {
    ...
    "13": {"file": "13-ui-copy-writer.md",            "name": "UI Copy Writer",            "provider": "claude", "model": "claude-sonnet-5"},
    "14": {"file": "14-ui-copy-reviewer.md",          "name": "UI Copy Reviewer",          "provider": "claude", "model": "claude-haiku-4-5"},
}
...
PIPELINE = {
    ...
    "13": {"validator": "strings",   "reviewer": "14"},
}
...
def list_agents():
    ...
    for code, info in AGENT_REGISTRY.items():
        lane = "Claude (personal)" if info["provider"] == "claude" else "Antigravity/Gemini Pro"
...
def _run(cmd: List[str], timeout: int, extra_env: Optional[Dict] = None,
         stdin_text: Optional[str] = None) -> str:
    """Runs a subprocess, failing loud on any error. Never fabricates output."""
    env = os.environ.copy()
    # The ELVTR team login used to be selected via CLAUDE_CONFIG_DIR; that
    # subscription is cancelled, so scrub any inherited value and let the CLI
    # resolve its default (personal) config.
...
def call_claude(system_prompt: str, user_prompt: str, model: str, timeout: int) -> Tuple[str, Dict]:
    """Calls the `claude` CLI headless on the machine-default (personal) login."""
    # The prompt goes on stdin, not in argv. Linux caps a single argument at
    # 128 KB regardless of how much room the whole argument list has, so passing
    # a large injected context as `-p <text>` fails with "argument list too
    # long" — a limit that has nothing to do with the model's context window and
    # everything to do with how the process was started.
    cmd = [
        "claude", "-p",
    ...
    out = _run(cmd, timeout, stdin_text=user_prompt)
    try:
        data = json.loads(out)
...
def agy_models(timeout: int = 180) -> List[str]:
    ...
    if _AGY_MODELS is None:
        ...
        # Each row is "<id>\t<display name>", preceded by a progress line with
        # no tab. Keeping whole rows made the membership test compare a bare id
        # against "id<TAB>Display Name", so every model read as unavailable
        # while the error message listed it as present.
        _AGY_MODELS = [ln.split("\t", 1)[0].strip()
...
def run_pipeline(gen_key: str, user_input: str, extra_notes_arg: Optional[str],
    ...
    print(f"🔑 Generator lane: {'Claude (personal)' if gen_info['provider']=='claude' else 'Antigravity/Gemini Pro'} | Model: {gen_info['model']}")
    ...
    if hard:
        # Keep the rejected artifact. Diagnosing why a generator could not
        # satisfy the gate needs the thing it produced, and discarding it makes
        # every failure a guess about which rule was unteachable.
        if parsed is not None and output:
            reject = OUTPUT_DIR / (Path(output).stem + ".rejected.json")
            reject.write_text(json.dumps(parsed, indent=2, ensure_ascii=False) + "\n",
                              encoding="utf-8")
            print(f"   Rejected artifact kept at {reject.name} for diagnosis.")
    ...
    if output:
        # The artifact alone is not importable. Both verdicts are written beside
        # it so the import stage has something to check: without them the gate
        # and the reviewer are opinions nobody recorded, and provenance.py has
        # nothing to stamp. Named as siblings of the artifact so the pair travels
        # together.
        ...
        (OUTPUT_DIR / f"{stem}.gate.json").write_text(json.dumps({
            "kind": cfg["validator"], "status": "PASS",
            "error_count": 0, "warning_count": len(warns), "errors": warns,
            "attempts": attempt,
    ...
    if output:
        stem = output[:-5] if output.endswith(".json") else output
        print(f"\n✅ Saved: {output}, {stem}.gate.json, {stem}.review.json")
        print(f"   Not importable yet — record provenance and approve first:")
        print(f"     provenance.py stamp   --file {output} --gate-report {stem}.gate.json "
              f"--review {stem}.review.json")
        print(f"     provenance.py approve --file {output} --note \"...\"")
...
def run_room_pipeline(room_brief: str, retries: int, timeout: int, output_prefix: Optional[str]):
    ...
    lane03 = "Claude (personal)" if i03["provider"] == "claude" else "Antigravity/Gemini Pro"
...
def main():
    ...
    lane = "Claude (personal)" if provider == "claude" else "Antigravity / Gemini Pro"
