...
def validate_room(room: Dict) -> List[Dict]:
    ...
    for i, eid in enumerate(path_ids):
        ...
        if support is None:
            _err(errors, "ERR_UNKNOWN_ELEMENT", f"critical_path names '{eid}', which is neither a door "
                 "nor anything that can be stood on", f"critical_path[{i}]")
            return errors
    ...
    for (aid, a), (bid, b) in zip(steps, steps[1:]):
        ...
        if gap > rr.GAP_GUARANTEED:
            _err(errors, "ERR_UNREACHABLE",
                 f"{aid} -> {bid} spans {gap:g}, past the {rr.GAP_GUARANTEED:g} guaranteed band", "critical_path")
