...
Reach says how far the character goes. These say how much space it occupies when
it gets there, which is a separate question and was missing until a room that
satisfied every reach figure turned out to be unclimbable.

*Read 2026-08-10 from `BP_GreyBoxCharacter`'s capsule and movement component.*

| | value |
|---|---|
| Capsule half-height | 88 → **height 176** |
| Capsule radius | 34 → **width 68** |
| `MaxStepHeight` | 45 |

**Headroom needed: 200** — the body plus room to move in it. `[TUNE]`; the floor
under it is 176, which is physics rather than preference.

**A jumping character occupies 301** — body 176 plus the 125 of a single jump.
This is the number that divides the two standard corridor heights: below it the
jump is clipped and the player cannot go over anything, above it evasion is
available. The heights themselves are in `roomspec.md`.

Two consequences worth stating, because both were violated by every room built
before they were written down:

- Vertical spacing is measured surface to surface, so the platform above eats its
  own thickness out of the space you stand in. Ledges 200 apart and 40 thick
  leave **160** of air for a 176-tall body.
- A climb cannot stack. Headroom of 200 with a rise of at most 200 would need a
  platform of zero thickness, so consecutive steps have to alternate sideways.
