...
class TheLawTravelsWithTheVerdict(unittest.TestCase):
    """A PASS is only as current as the rules that issued it.

    This class exists because a room approved in August walked through the
    importer while failing the current gate with eight softlocks — the record
    said PASS because PASS had been true once.
    """
    ...
    def _approved(self, rules_sha):
        artifact = self.dir / "room.json"
        artifact.write_text(json.dumps(ROOM))
        gate = dict(GATE_PASS)
        if rules_sha is not None:
            gate["rules_sha256"] = rules_sha
        gate_path = self.dir / "gate.json"
        gate_path.write_text(json.dumps(gate))
        review_path = self.dir / "review.json"
        review_path.write_text(json.dumps(REVIEW_PASS))
        pv.stamp(artifact, gate_path, review_path)
        pv.approve(artifact, "test")
        return artifact

    def test_a_verdict_from_the_current_rules_is_honoured(self):
        current = pv.current_rules_fingerprint()
        if current is None:
            self.skipTest("rules modules not importable here")
        pv.check(self._approved(current))          # must not raise

    def test_a_verdict_from_superseded_rules_is_refused(self):
        if pv.current_rules_fingerprint() is None:
            self.skipTest("rules modules not importable here")
        artifact = self._approved("f" * 64)
        with self.assertRaises(pv.ProvenanceError) as c:
            pv.check(artifact)
        self.assertIn("rules have changed", str(c.exception))

    def test_an_old_record_without_a_fingerprint_still_checks(self):
        # Records stamped before the fingerprint existed carry none. Refusing
        # them all would invalidate history retroactively; the artifact hash
        # remains their bond, and re-stamping upgrades them.
        pv.check(self._approved(None))             # must not raise
