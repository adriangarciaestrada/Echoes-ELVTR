...
export interface Snapshot {
  ...
  offers: string[];
  offerTiers: number[];
  /** How many times the shelf has been dealt; see the tier-mutation rule. */
  deals: number;
  removed: string[];
  buffs: number;
...
export function inspect(snap: Snapshot, prev: Snapshot | null): Violation[] {
  ...
  if (prev) {
    ...
    if (prev.phase === "market" && snap.phase === "market" &&
        ...
        // Only when the shelf was NOT dealt again in between. A fresh deal puts
        // the same relic back in the same slot at a different tier about 12% of
        // the time, which looks identical to a desync from the shelf alone —
        // 1241 of these were reported the first run after the real desync was
        // fixed, and every one of them was a re-deal.
        snap.deals === prev.deals) {
