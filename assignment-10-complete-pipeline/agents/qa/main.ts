...
/**
 * A duration in seconds, or the default if the flag is absent.
 *
 * It exits rather than falling back when the value is not a positive number:
 * `--browser --url ...` parsed the next flag as the duration, ran the browser
 * pass for NaN seconds — three tactic cycles instead of the usual twenty-odd —
 * and still printed "0 distinct findings, 12/12 rules held". A hollow run that
 * reads as a clean bill of health is worse than no run, so a bad duration is a
 * hard error and an unknown flag is refused outright.
 */
const flag = (name: string, fallback: number): number => {
  ...
  if (!Number.isFinite(n) || n <= 0) {
    console.error(`--${name} wants a positive number of seconds, got ${argv[i + 1] ?? "nothing"}`);
    process.exit(2);
  }
  return n;
};
const KNOWN = new Set(["--core", "--browser", "--no-browser", "--headed", "--out"]);
for (const a of argv) {
  if (a.startsWith("--") && !KNOWN.has(a)) {
    console.error(`unknown flag ${a} (known: ${[...KNOWN].join(" ")}; the URL comes from LOOM_URL)`);
    process.exit(2);
  }
}
