"""
Deterministic checks for the narrative engine.
...
consistency claims ("does the ledger stay correct across 5+ turns", "does
prior history change later dialogue") are checked against the REAL saved
transcripts from an actual run, not a mocked assumption of what the model
would do — a model call is the one thing this file cannot make
deterministic, so the transcripts are checked as data instead of
re-invoking the model in the test.

    python3 narrative_engine.py --demo                 # writes output/demo_run.json (duty branch)
    python3 narrative_engine.py --demo --branch guilt   # writes output/guilt_run.json
...
def test_patch_sets_new_flags_the_schema_never_predefined():
    # The model does exactly this in real runs — see README.md's "surprising
...
def _assert_stays_consistent(data: dict) -> None:
    ...
    assert len(transcript) >= 5, "at least 5 turns"
    ...
    # contradiction this engine is built not to make.
    ...
    # "5+ turns of consistency" and "reactive to history, not just the latest
    # input" are two different claims, and this scenario's fork is built to
    # test the second without contradicting the first.
...
def test_duty_and_guilt_produce_different_outcomes():
    # This story's own fork: whether Kade's death was the player's order or
    # his own call. Checked here as a difference in tracked STATE, which is
    # what actually drives the dialogue difference turn to turn — the state
    # is the reason the dialogue can differ at all, not a side effect of it.
    # Both branches still end identically on the outward fact the story
    # exists to explain (Kade dead, player alone at Farwatch); only the
    # load-bearing flag for HOW differs.
    duty = _load("demo_run")
