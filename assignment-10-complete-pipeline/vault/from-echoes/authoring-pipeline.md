...
**Content with a spec** — relics, waves, text, tuning tables — enters only
...
- **The whole plan is computed before anything is written.** A spec that
  fails halfway must not leave half of itself behind.
